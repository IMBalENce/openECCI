    ...
    if file_type in ["FEI_SEM", "Zeiss_SEM"]:
        ...
        with TiffFile(filename) as tif:
            if tif.fei_metadata:
                ...
                st_x = tif.fei_metadata['Stage']['StageX'] - tif.fei_metadata['Beam']['BeamShiftX']
                st_y = tif.fei_metadata['Stage']['StageY'] - tif.fei_metadata['Beam']['BeamShiftY'] 
                ...
                st_x = tif.sem_metadata['ap_stage_at_x'][1]
                st_y = tif.sem_metadata['ap_stage_at_y'][1]
                st_z = tif.sem_metadata['ap_stage_at_z'][1]
                #TODO: add pixel size reader for Zeiss file format
...
def get_file_footprint(filename: str) -> str:
    ...
    * Kikuchipy .hdf5 file                      "kpy_hdf5"
    ...
    if file_extension in [".tif", ".tiff", ".TIFF"]:
        try:
            ebsp_metadata = get_hkl_metadata(filename=filename,)
        ...
        with h5.File(filename, "r") as f:
            try:
                ...
                if manufacturer != "Oxford Instruments":
                    raise KeyError
                else: file_type = "hkl_hdf5"
            except KeyError:
                try:
                    manufacturer = f[" Manufacturer"][:][0].decode()
                    if manufacturer != "EDAX":
                        raise KeyError
                    else: file_type = "tsl_hdf5"
                except KeyError:
                    try:
                        manufacturer = f['EMheader/EBSDmaster/ProgramName'][:][0].decode()
                        if manufacturer not in ["EMEBSDmaster.f90"]:
                            raise KeyError
                        else: file_type = "emsoft_hdf5"
                    except KeyError:
                        try:
                            manufacturer = f["manufacturer"][:][0].decode()
                            if manufacturer != "kikuchipy":
                                raise KeyError
                            else: file_type = "kpy_hdf5"
                        except KeyError:
                            raise IOError("{filename} is not supported format")
...
def get_hkl_metadata(filename: str) -> dict:
    ...
    with TiffFile(filename) as tif:
        ...
        ebsd_metadata = {myroot[0][i].tag:myroot[0][i].text for i in range(len(myroot[0]))}
...
def save_align_points(points1: np.ndarray, 
