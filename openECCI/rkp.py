...
from openECCI.io import load_xmap, get_sem_metadata, get_file_footprint
import numpy as np
...
def get_sim_rkp(RKP_masterpattern, 
    ...
    rkp_sim = RKP_masterpattern.get_patterns(
        rotations = xtal_rotation *tiltZ_corr*tiltY_corr*tiltX_corr * st_rot*st_tilt,
...
def interactive_xmap(xmap: crystal_map, 
                     phase_name: str, 
                     overlay: str = None,
    ...
    def on_click(event):
        if event.dblclick:
            ...
            try:
                ...
                x_pos = 0
                y_pos = 0
            [Eu1, Eu2, Eu3] = np.rad2deg(Rotation.to_euler(xmap[int(y_pos), int(x_pos)].orientations))[0]
            plt.plot(x_pos, y_pos, '+', c='black', markersize=12, markeredgewidth=2)
            plt.title(f"Euler angles: {np.round(Eu1, 2)}, {np.round(Eu2, 2)}, {np.round(Eu3, 2)}")
...
def rkp_angle_finder(RKP_masterpattern, 
                     xtal_rotation: Rotation,
                     phasename: str, 
                     ref_ECP: str,
                     corr_angles: list,
    ...
    """
    fig, (ax1, ax2)= plt.subplots(nrows=1, ncols=2, subplot_kw=dict(projection="plot_map"), figsize=(16,8))
    st_rot = Rotation.from_axes_angles([0, 0, 1], -st_rot_angle, degrees=True)
    st_tilt = Rotation.from_axes_angles([0, 1, 0], -st_tilt_angle, degrees=True)
    ...
    phase=phasename, hkl=[[1, 1, 1], [2, 0, 0], [2, 2, 0], [3, 1, 1]]
    )
    ...
    sim_RKP_lowMag = get_sim_rkp(RKP_masterpattern, 
                                 xtal_rotation, 
                                 st_rot_angle, 
                                 st_tilt_angle,
                                 corr_angles,
    ...
    ax2.cla()
    sim_RKP_hiMag = get_sim_rkp(RKP_masterpattern, 
                                xtal_rotation, 
                                st_rot_angle, 
                                st_tilt_angle,
                                corr_angles,
    ...
    def on_click(event):
        if event.dblclick:
            ...
            if stage_mode == "rot-tilt":
                ...
                polar_rkp = math.atan( math.sqrt(distance_x**2 + distance_y**2) / distance_l)
                ax2.set_title(
                f"""
...
def get_xmap_image(xmap: crystal_map, 
                     phase_name: str, 
                     overlay: str = None,) -> np.ndarray:
    """
    ...
    if overlay is not None:
        ...
        rescaled_overlay = (overlay_1dim - overlay_min) / (np.nanmax(overlay_1dim) - overlay_min)
        n_channels = 3
